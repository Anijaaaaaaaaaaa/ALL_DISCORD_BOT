import asyncio, re, random, discord, os, aiomysql
...
class auto_bot(commands.Cog):
    ...
    async def ok(self, ctx):
        ...
        while True:
            ...
            embed.set_footer(text=f"制作者:{self.bot.get_user(304932786286886912).mention} | 編集者: {self.bot.get_user(574166391071047694).mention}, {self.bot.get_user(204966469593858048).mention}",)
            ...
            try:
                react = await self.bot.wait_for('reaction_add', timeout=20, check=predicate(msg, ctx.message.author, l, r, self.bot))
        ...
        try:
            ...
            if not role_name:
                ...
                while True:
                    ...
                    if r:
                        await msg.add_reaction(right)
                    react = await self.bot.wait_for('reaction_add', timeout=10, check=predicate(msg, ctx.message.author, l, r, self.bot))
                    if react[0].emoji == left:
                        index -= 1
    ...
    async def on_message(self, message):
        if len(message.embeds) != 0 and message.author.id == 526620171658330112 and message.embeds[0].title:
                ...
                if message.embeds[0].title[-7:] == "のステータス:":
                    ...
                    async with aiomysql.connect(host="localhost", user="root", db="role", password="") as conn:
                        async with conn.cursor() as cur:
                            ...
                            for lower, upper, role_id in await cur.fetchall():
                    ...
                    role_id = next((role_id for role_id, lu in role_level.items() if (lambda x: lu[0] <= x <= lu[1])(level)), None)
                    role = discord.utils.get(message.guild.roles,id=role_id)
                    next_level = 0
            ...
            except (IndexError or TypeError or AttributeError or ValueError):
                pass

        if any([True for s in ["DISCORD.GG", "DISBOARD.ORG", "BIT.LY", "DISCORDAPP", "INVITE.GG", "DISCORD.COM"] if s in message.content.upper()]):
            if message.guild.id == 337524390155780107 and not (message.channel.id == 421954703509946368 or message.channel.id == 539805974672834570):
        ...
        if message.channel.name == "tao-global":
            async with aiomysql.connect(host="localhost", user="root", db="role", password="") as conn:
                async with conn.cursor() as cur:
                    ...
                    if any([True for s in ['.jpg', '.png', '.jpeg', '.tif', '.tiff', '.bmp', '.gif', '.mp4'] if s in message.content]):
                        ...
                        embed = discord.Embed(message.content, timestamp=message.created_at)
